...
SGLight GenerateVSGL(const float4 positionAvg, const float3 axisAvg, const float3 power)
{
	...
	const float variance = max((positionAvg.w - dot(positionAvg.xyz, positionAvg.xyz)) / 2.0, FLT_MIN);
...
void main(const uint2 threadID : SV_DispatchThreadID, const uint groupIndex : SV_GroupIndex)
{
	...
	for (uint y = 0; y < RSM_WIDTH; y += THREAD_GROUP_WIDTH)
	{
		for (uint x = 0; x < RSM_WIDTH; x += THREAD_GROUP_WIDTH)
		{
			...
			const float axisLength = 0.5293165801288393926136314; // Equivalent to VMFSharpnessToAxisLength(SG_CUT_COSINE_SHARPNESS).
			...
			const SGLobe sg = SGReflectionLobe(-direction, normal, squaredRoughness);
			const float3 axis = sg.axis;
			const float axisLength = VMFSharpnessToAxisLength(sg.sharpness);
			const float lobeOverPDF = SmithGGXLobeOverPDF(-direction, axis, normal, squaredRoughness); // Fresnel = 1 in this implementation.

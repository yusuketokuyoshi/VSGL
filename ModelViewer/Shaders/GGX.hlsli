...
float GGX(const float3 m, const float2x2 roughnessMat)
{
	...
	const float length2 = dot(m.xy, mul(roughnessMatAdj, m.xy)) / det + m.z * m.z; // TODO: Use Kahan's algorithm for mul and dot. [https://pharr.org/matt/blog/2019/11/03/difference-of-floats]

	return (m.z > 0.0) ? 1.0 / (M_PI * sqrt(det) * (length2 * length2)) : 0.0;
}

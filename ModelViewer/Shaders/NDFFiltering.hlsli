...
// dndu, dndv: screen-space derivatives of interpolated vertex normals.
// roughness: GGX (or Beckmann) alpha roughness.
float2 IsotropicNDFFiltering(const float3 dndu, const float3 dndv, const float2 roughness)
{
    ...
    const float kernelRoughness2 = SIGMA2 * (dot(dndu, dndu) + dot(dndv, dndv));
